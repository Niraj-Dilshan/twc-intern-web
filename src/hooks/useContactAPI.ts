...
const useContactAPI = () => {
  const { addContact, updateContact, deleteContact, setError } =
    useContactStore();
  ...
  const addContactMutation = useMutation(async (contact) => {
    try {
  ...
  const deleteContactMutation = useMutation(async (contactId: number) => {
    try {
